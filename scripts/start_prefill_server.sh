...
if [[ ! "$MOE_FUSED_EP_KERNEL_MIN_TOKENS" =~ ^[0-9]+$ ]]; then
  echo "ERROR: MOE_FUSED_EP_KERNEL_MIN_TOKENS must be a non-negative integer, got '$MOE_FUSED_EP_KERNEL_MIN_TOKENS'." >&2
  exit 2
fi
...
case "${MOE_FUSED_EP_V2_SPLIT_ACTIVATION_GATHER,,}" in
  1|true) MOE_FUSED_EP_V2_SPLIT_ACTIVATION_GATHER=1 ;;
  0|false) MOE_FUSED_EP_V2_SPLIT_ACTIVATION_GATHER=0 ;;
  *)
    echo "ERROR: MOE_FUSED_EP_V2_SPLIT_ACTIVATION_GATHER must be a boolean, got '$MOE_FUSED_EP_V2_SPLIT_ACTIVATION_GATHER'." >&2
    exit 2
    ;;
esac
